...
- 질문 : 커버링 인덱스를 사용하면 확실히 실제 데이터에 엑세스 하는 걸 없앨 수 있어서 select 가 빨라지지만, 이 방식을 실무에서도 많이 사용하는지? 인덱스에 데이터를 포함하는 것은 결국 마치 redis 같은 메모리 db를 사용하는 것과 비슷한 방법 같은데, 실무에서도 많이 활용하는지가 궁금합니다.
